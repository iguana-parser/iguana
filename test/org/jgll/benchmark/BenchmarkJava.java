...
//	private static Grammar grammar = new LayoutWeaver().transform(new EBNFToBNF().transform(JavaCharacterLevel.grammar));
	private static Nonterminal start = Start.from(Nonterminal.withName("VariableInitializer"));
...
//		IguanaBenchmark.builder(grammar, start).addFile("/Users/anastasiaizmaylova/git/diguana/test.java").setRunCount(5).build().run();
