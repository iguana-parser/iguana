...
public class Test13 {
	...
	private SPPFNode getSPPF(GrammarSlotRegistry registry) {
		...
		NonterminalNode node29 = factory.createNonterminalNode("D", 0, 1, 1).init();
		PackedNode node30 = factory.createPackedNode("D ::= .", 1, node29);
		node30.addChild(node12);
		node29.addChild(node30);
		node2.addChild(node3);
		node2.addChild(node29);
		PackedNode node32 = factory.createPackedNode("S ::= A B C D .", 0, node1);
		IntermediateNode node33 = factory.createIntermediateNode("S ::= A B C . D", 0, 0).init();
		node33.addChild(node23);
