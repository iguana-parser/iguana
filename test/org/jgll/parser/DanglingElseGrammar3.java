...
public class DanglingElseGrammar3 {
	...
	private SPPFNode getExpectedSPPF(GrammarSlotRegistry registry) {
		...
		IntermediateNode node3 = factory.createIntermediateNode("S ::= a S b . S", 0, 4).init();
		PackedNode node4 = factory.createPackedNode("S ::= a S b . S", 3, node3);
		IntermediateNode node5 = factory.createIntermediateNode("S ::= a S . b S", 0, 3).init();
		...
		NonterminalNode node15 = factory.createNonterminalNode("S", 0, 4, 5).init();
		...
		NonterminalNode node20 = factory.createNonterminalNode("S", 0, 1, 5).init();
		PackedNode node21 = factory.createPackedNode("S ::= a S b S .", 4, node20);
		IntermediateNode node22 = factory.createIntermediateNode("S ::= a S b . S", 1, 4).init();
		PackedNode node23 = factory.createPackedNode("S ::= a S b . S", 3, node22);
		IntermediateNode node24 = factory.createIntermediateNode("S ::= a S . b S", 1, 3).init();
