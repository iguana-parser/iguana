...
public class Test10 {
    ...
    static Rule r1 = Rule.withHead(S).addSymbols(A).build();
    static Rule r2 = Rule.withHead(A).addSymbols(Plus.from(B)).build();
    static Rule r3 = Rule.withHead(B).addSymbols(b).build();

    static Start start = Start.from(S);
    private static Grammar grammar = EBNFToBNF.convert(new DesugarStartSymbol().transform(Grammar.builder().addRules(r1, r2, r3).setStartSymbol(start).build()));
    ...
    public static void record() {
        ...
        TestRunner.record(grammar, input1, 1, path + "/Test10");
        TestRunner.record(grammar, input2, 2, path + "/Test10");
