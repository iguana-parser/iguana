...
public class Test13 {
    ...
    static Nonterminal B = Nonterminal.withName("A");
    static Nonterminal C = Nonterminal.withName("A");
    static Nonterminal D = Nonterminal.withName("A");
    ...
    static Terminal d = Terminal.from(Char.from('d'));

    static Rule r1 = Rule.withHead(S).addSymbols(A, B, Plus.from(C), D).build();
    ...
    static Rule r5 = Rule.withHead(D).addSymbols(d).build();

    static Start start = Start.from(S);
    private static Grammar grammar = EBNFToBNF.convert(new DesugarStartSymbol().transform(Grammar.builder().addRules(r1, r2, r3, r4, r5).setStartSymbol(start).build()));

    private static Input input1 = Input.fromString("abcd");
    private static Input input2 = Input.fromString("abcccccd");

    @BeforeClass
    public static void record() {
        System.out.println(grammar);
        String path = Paths.get("test", "resources", "grammars", "ebnf").toAbsolutePath().toString();
        TestRunner.record(grammar, input1, 1, path + "/Test13");
        TestRunner.record(grammar, input2, 2, path + "/Test13");
    }
