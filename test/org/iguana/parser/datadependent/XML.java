...
.addRule(Rule.withHead(Nonterminal.builder("Name").build()).addSymbol(Terminal.from(Alt.builder(CharacterRange.builder(97, 122).build()).build())).addSymbol(Star.builder(Terminal.from(Alt.builder(CharacterRange.builder(97, 122).build()).build())).build()).setLayoutStrategy(NO_LAYOUT).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
.build();
