...
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("STag").build()).addSymbol(Terminal.builder(iguana.regex.Seq.builder(Char.builder(60).build()).build()).build()).addSymbol(new Nonterminal.Builder("Name").setLabel("n").build()).addSymbol(new Star.Builder(new Nonterminal.Builder("Attribute").build()).build()).addSymbol(Terminal.builder(iguana.regex.Seq.builder(Char.builder(62).build()).build()).build()).addSymbol(new Return.Builder(AST.yield("n")).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
...
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("ETag").addParameters("s").build()).addSymbol(Terminal.builder(iguana.regex.Seq.builder(Char.builder(60).build(), Char.builder(47).build()).build()).build()).addSymbol(new Conditional.Builder(new Nonterminal.Builder("Name").setLabel("n").build(),AST.equal(AST.yield("n"),AST.var("s"))).build()).addSymbol(Terminal.builder(iguana.regex.Seq.builder(Char.builder(62).build()).build()).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
// Attribute ::= (_)  {UNDEFINED,-1,NON_REC} PREC(1,1) 
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("Attribute").build()).addSymbol(Terminal.builder(iguana.regex.Seq.builder(Char.builder(95).build()).build()).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
