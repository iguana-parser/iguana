...
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("D").build()).addSymbol(Terminal.builder(Seq.builder(Char.builder(100).build()).build()).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
// S ::= A do var m = 3; C B do var n = 4; D  {UNDEFINED,-1,NON_REC} PREC(1,1) 
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("S").build()).addSymbol(new Code.Builder(new Nonterminal.Builder("A").build(),AST.varDeclStat(AST.varDecl("m",AST.integer(3)))).build()).addSymbol(new Nonterminal.Builder("C").build()).addSymbol(new Code.Builder(new Nonterminal.Builder("B").build(),AST.varDeclStat(AST.varDecl("n",AST.integer(4)))).build()).addSymbol(new Nonterminal.Builder("D").build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
// C ::= (c)  {UNDEFINED,-1,NON_REC} PREC(1,1) 
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("C").build()).addSymbol(Terminal.builder(Seq.builder(Char.builder(99).build()).build()).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
// B ::= (b) do z = x; (;)  {UNDEFINED,-1,NON_REC} PREC(1,1) 
.addRule(RuntimeRule.withHead(new Nonterminal.Builder("B").build()).addSymbol(new Code.Builder(Terminal.builder(Seq.builder(Char.builder(98).build()).build()).build(),AST.stat(AST.assign("z",AST.var("x")))).build()).addSymbol(Terminal.builder(Seq.builder(Char.builder(59).build()).build()).build()).setRecursion(Recursion.NON_REC).setAssociativity(Associativity.UNDEFINED).setPrecedence(-1).setPrecedenceLevel(PrecedenceLevel.from(1,1,-1,false,false,false,false)).build())
.build();
