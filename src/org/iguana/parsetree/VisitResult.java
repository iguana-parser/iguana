...
public abstract class VisitResult {
    ...
    public static class CreateParseTreeVisitor<T> implements CreateNodeVisitor<java.util.List<T>> {
        ...
        public java.util.List<T> visit(Empty result, PackedNode packedNode) {
            ...
            return CollectionsUtil.list(parseTreeBuilder.nonterminalNode(rule, (java.util.List<T>) result.getValues(),
                packedNode.getLeftExtent(), packedNode.getRightExtent()));
        }
        ...
        public java.util.List<T> visit(List result, PackedNode packedNode) {
            ...
            RuntimeRule rule = packedNode.getGrammarSlot().getRule();
            return CollectionsUtil.list(parseTreeBuilder.nonterminalNode(rule, values, packedNode.getLeftExtent(),
                packedNode.getRightExtent()));
        }

        @Override
        public java.util.List<T> visit(EBNF result, PackedNode packedNode) {
            T ebnfNode = parseTreeBuilder.metaSymbolNode(result.getSymbol(), (java.util.List<T>) result.getValues(),
                packedNode.getLeftExtent(), packedNode.getRightExtent());
